...
def main():
    ...
    playlists = pd.DataFrame(
        [
            # Rock Playlist
            ("PL_1", "Bohemian Rhapsody"),
            ("PL_1", "Stairway to Heaven"),
            ("PL_1", "Hotel California"),
            # Pop Playlist
            ("PL_2", "Thriller"),
            ("PL_2", "Billie Jean"),
            ("PL_2", "Hotel California"),  # Bridge song!
            # Metal Playlist
            ("PL_3", "Master of Puppets"),
            ("PL_3", "Enter Sandman"),
            # Discovery
            ("PL_4", "Enter Sandman"),
            ("PL_4", "Bohemian Rhapsody"),  # Bridge between Metal and Rock
        ],
    ...
    with graph:
        ...
        graph.load_interactions(data=playlists, context_col="playlist_id", node_col="song_name")
        ...
        related = graph.rank_nodes(seed_nodes=[seed_song], steps=3, scoring="probability", limit=5)

        print(related)

        if "Hotel California" in related["node"].to_pylist():
            print("\n-> Discovery: Found 'Hotel California' (Rock/Pop) from Metal start!")
            print(
                "   Path: Master of Puppets -> Enter Sandman -> Bohemian Rhapsody -> Hotel California"
            )
